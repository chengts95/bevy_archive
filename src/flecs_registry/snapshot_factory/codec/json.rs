...
macro_rules! gen_all {
    // case: no wrapper
    (emplace, $t:ty) => {
        (gen_export!(full, $t), gen_import!(emplace, $t))
    };
    // case: no wrapper
    ($mode:ident, $t:ty) => {
        (gen_export!($mode, $t), gen_import!($mode, $t))
    };

    // case: with wrapper
    (emplace, $t:ty, $t1:ty) => {
        (gen_export!(full, $t, $t1), gen_import!(emplace, $t, $t1))
    };
    // case: with wrapper
    ($mode:ident, $t:ty, $t1:ty) => {
        (gen_export!($mode, $t, $t1), gen_import!($mode, $t, $t1))
    };
}
